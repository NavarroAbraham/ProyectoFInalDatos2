...
class Nodo(QGraphicsEllipseItem):
    def __init__(self, x, y, radius, id, app, color="lightblue"):
        # Inicializa el nodo como un círculo con un radio dado
        super().__init__(-radius, -radius, 2 * radius, 2 * radius)
        self.setBrush(QtGui.QBrush(QtGui.QColor(color)))  # Color de fondo del nodo
        ...
        self.setFlag(QGraphicsEllipseItem.ItemSendsGeometryChanges)  # Notifica cambios de posici��n
...
class Arista(QGraphicsLineItem):
    def __init__(self, nodo1, nodo2, peso, scene, color="black"):
            ...
            for i in range(num_nodos):
                for j in range(i + 1, num_nodos):
                    ...
                    if peso > 0:  # Si hay una conexión (peso > 0)
                        nodo1 = self.nodos[i]
                        nodo2 = self.nodos[j]
                        arista = Arista(nodo1, nodo2, peso, self.scene, color="blue")  # Cambia el color de la arista
                        self.aristas.append(arista)
